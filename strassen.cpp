// Mojtaba alehosseini
// Alehoseini.Mojtaba@gmail.com

#include <iostream>
using namespace std;

const int N = 4;     //andaze matris
// N bayad tavani az 2 bashad (N=2^k)
// agar N tavani az 2 nabud be tedad morede niaz satr o sotun ezafe miknim
// in barname be surate khodkar satr o sotun ezafe nmiknd
...
//          zarb
template<typename T>
void Matrix_Multiply(T A[][N], T B[][N], T C[][N]) {  //mohasebe A*B->C
     //output(2,A);
     //output(2,B);
     for(int i=0; i<2; i++) {
        for(int j=0; j<2; j++) {
